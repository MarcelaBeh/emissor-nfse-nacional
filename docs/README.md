# Documentação - NFSe Nacional

## 📚 Guias

| Documento | Descrição |
|-----------|-----------|
| [GUIA_IMPLEMENTACAO.md](GUIA_IMPLEMENTACAO.md) | Guia completo de uso com exemplos de código |
| [ARQUITETURA.md](ARQUITETURA.md) | Arquitetura do sistema, camadas e padrões |
| [SEGURANCA_COMPLIANCE.md](SEGURANCA_COMPLIANCE.md) | Diretrizes de segurança e compliance |

## 📁 Referências

| Arquivo | Descrição |
|---------|-----------|
| `repositories/codigos-erro-nfse.txt` | Códigos de erro da Receita Federal |
| `repositories/danfse-spec.pdf` | Especificação técnica DANFSe |

## 🔗 Links Externos

- [Portal NFSe Nacional](https://www.nfse.gov.br/)
- [Ambiente de Homologação](https://www.producaorestrita.nfse.gov.br/)
- [NFePHP](https://github.com/nfephp-org)
- [PSR Standards](https://www.php-fig.org/psr/)
