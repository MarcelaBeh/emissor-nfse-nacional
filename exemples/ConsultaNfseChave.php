...
try {
    $response = $facade->consultarPorChave($chave, encoding: true);
    echo 'Sucesso: ' . ($response->success ? 'Sim' : 'Nao') . PHP_EOL;

    if ($response->xml !== null) {
        file_put_contents('consulta-nfse-retorno.xml', $response->xml);
        echo 'XML salvo em consulta-nfse-retorno.xml' . PHP_EOL;
    }
} catch (\Throwable $e) {
